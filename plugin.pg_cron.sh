...
show_usage() {
    ...
    echo ""
    echo "示例:"
    echo "  $0 info                    # 使用默认版本 ${DEFAULT_PG_CRON_VERSION}"
    echo "  $0 info 1.6.0              # 指定版本 1.6.0"
    echo "  $0 build host 16.15        # 使用默认版本 ${DEFAULT_PG_CRON_VERSION}"
    echo "  $0 build host 16.15 1.6.0  # 指定版本 1.6.0"
...
get_version() {
    ...
    if [ -n "$version" ]; then
        echo "$version"
    elif [ -n "$PG_CRON_VERSION" ]; then
        echo "$PG_CRON_VERSION"
    else
        echo "$DEFAULT_PG_CRON_VERSION"
    fi
}
...
build="$basedir/build/$triple/$version/pg_cron_v${pg_cron_version}"
cache="$basedir/cache"
...
# ==========================================================
download_pg_cron() {
    download "$pg_cron_tar" "https://github.com/citusdata/pg_cron/archive/refs/tags/v${pg_cron_version}.tar.gz"
}
...
# ==========================================================
build_pg_cron() {
    log="$build/pg_cron.log"
    rm -f "$log"
    
    cd "$basedir/build/$triple/$version"
    rm -rf "pg_cron_v${pg_cron_version}"
    mkdir -p "pg_cron_v${pg_cron_version}"
    tar xf "$cache/$pg_cron_tar" -C "pg_cron_v${pg_cron_version}" --strip-components 1
    cd "pg_cron_v${pg_cron_version}"

    log_with_time "配置编译环境: pg_cron v${pg_cron_version}"
    ...
    log_with_time "编译中: pg_cron v${pg_cron_version}"
...
package_pg_cron() {
    ...
    pkg_name="pg_cron-v${pg_cron_version}-pg${pg_major}.${arch}.tar.gz"
    ...
    if ! make USE_PGXS=1 \
            ...
            install >>"$build/pg_cron.log" 2>>"$build/pg_cron.log"; then
        log_with_time "安装失败!"
        cat "$build/pg_cron.log" | tail -50
        exit 1
    fi
    ...
    echo ""
    echo "=========================================="
    echo "完成! pg_cron v${pg_cron_version} 已打包"
    echo "   包路径: $pkg_path"
    echo "   包内容:"
    tar -tzf "$pkg_path"
    echo "=========================================="
...
# build 模式: 主流程
# ==========================================================
log_with_time "开始构建 pg_cron v${pg_cron_version} (目标: $triple, PG: $version)"

download_pg_cron

build_dir=$(build_pg_cron)

package_pg_cron "$build_dir"
